...
export default function Phase2Vorpruefung({ onNext }) {
  ...
  return (
    <div className="max-w-2xl flex flex-col gap-6">
      <div>
        <h2 className="text-lg font-semibold text-claimm-dark mb-1">Vorprüfung</h2>
        <p className="text-sm text-claimm-dark/60">Anspruchsgrund, Beschreibung und Nachweise erfassen.</p>
      </div>

      <div className="bg-white border border-claimm-dark/10 rounded-md p-5 flex flex-col gap-4">
        <div>
      ...
      <div className="bg-white border border-claimm-dark/10 rounded-md p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-claimm-dark">Nachweischeckliste</h3>
          <span className="text-sm font-semibold text-claimm-red">{vollstaendigkeit}% vollständig</span>
        </div>
        <div className="w-full h-1.5 bg-claimm-dark/10 rounded-full mb-4 overflow-hidden">
        ...
        <ul className="flex flex-col gap-2">
          {checkliste.map((n) => (
            <li key={n.id} className="flex items-center gap-2.5">
              ...
              />
              <span className="text-sm text-claimm-dark">{n.label}</span>
              <span className={`text-xs ml-auto ${n.status === 'bestätigt' ? 'text-green-700' : 'text-claimm-red'}`}>
                {n.status}
              </span>
            </li>
          ))}
