...
export default function Phase5Verhandlung({ onNext }) {
  ...
  return (
    <div className="max-w-2xl flex flex-col gap-6">
      <div>
        <h2 className="text-lg font-semibold text-claimm-dark mb-1">Verhandlung</h2>
        <p className="text-sm text-claimm-dark/60">Verhandlungsverlauf und interne Notizen.</p>
      </div>

      <div className="bg-white border border-claimm-dark/10 rounded-md p-5">
        <h3 className="text-sm font-semibold text-claimm-dark mb-4">Verhandlungshistorie</h3>
        <div className="flex flex-col gap-3">
          {verhandlungsHistorie.map((v) => (
            <div key={v.bezeichnung}>
              <div className="flex justify-between text-xs text-claimm-dark/60 mb-1">
                <span>
                  {v.bezeichnung} · {v.datum}
                </span>
                <span className="font-medium text-claimm-dark">{formatEUR(v.betrag)}</span>
              </div>
              <div className="w-full h-3 bg-claimm-dark/5 rounded-full overflow-hidden">
                <div
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-md p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-semibold text-amber-900">Interne Notizen (nicht extern sichtbar)</h3>
        </div>
        <textarea
          className="w-full border border-amber-300 rounded px-3 py-2.5 text-sm text-claimm-dark bg-white focus:outline-none focus:ring-2 focus:ring-amber-400"
          rows={5}
          value={notizen}
